...
type ShopifyOrder = {
  ...
  line_items: LineItem[]; shipping_address?: ShopifyAddress;
};
...
function normalizeProvince(country2: string, province?: string) {
  const p = String(province || "").trim();
  if (!p) return "";
  if (country2 === "US") return US_STATE[p.toLowerCase()] || (p.length === 2 ? p.toUpperCase() : "");
  if (country2 === "CA") return CA_PROV[p.toLowerCase()] || (p.length === 2 ? p.toUpperCase() : "");
...
function buildCreateLabelPayload(order: ShopifyOrder){
  ...
  const province2 = normalizeProvince(country2, addr.province);
...
export default async function handler(req:NextApiRequest,res:NextApiResponse){
  ...
  try{
    ...
    // validazione minima: US/CA richiedono provincia a 2 lettere
    if ((payload.receiver.country==="US" || payload.receiver.country==="CA")
        && (!payload.receiver.province || payload.receiver.province.length!==2)) {
      res.status(400).json({ ok:false, error:"local-validation", missing:["receiver.province(US/CA 2-letter)"] });
      return;
    }

    console.log("[SPRO] payload", { ref: payload.merchant_reference, to_country: payload.receiver.country, province: payload.receiver.province, pkg: payload.packages[0] });
    ...
    try{
      ...
      if (e?.code===1011 || e?.message==="SPRO_NO_CREDITS"){
        ...
        if (newTags !== (order.tags||"")) await updateOrderTags(order.id, newTags);
        res.status(200).json({ ok:false, skipped:"no-credits" });
        return;
      }
      throw e;
    }
